...
@Component({
  ...
  template: `
    <ion-card class="flex items-center gap-4">
      <div class="absolute font-bold text-xl drop-shadow-[0_2px_0px_rgba(255,255,255,1)] top-3 left-24" [ngClass]="numberColor()">{{player().jerseyNumber}}</div>
      <app-headshot [playerId]="player().personId" [width]="110" />
  ...
  `,
  imports: [IonicModule, HeadshotComponent, LogoComponent, NgClass]
})
export class PlayerItemComponent {
  player = input.required<Player>()

  numberColor = computed(() => {
    return getTextTeam(this.player().teamAbbreviation!);
  })
}
